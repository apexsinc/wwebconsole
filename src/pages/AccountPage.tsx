...
export default function AccountPage() {
  ...
  // Account pages are unlisted (robots.txt also disallows /account).
  useEffect(() => {
    applyDocumentSeo({
      title: 'Account — Weatherlink Web Console',
      description: 'Manage your Weatherlink Web Console account.',
      path: '/account',
      indexable: false,
    });
  }, []);
  ...
  if (!authChecked) {
    return (
      <div className="min-h-screen bg-[#0a0d14] flex flex-col items-center justify-center gap-3" role="status" aria-live="polite">
        ...
        <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Loading console account details…</p>
      </div>
    );
  }
